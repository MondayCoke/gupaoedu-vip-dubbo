...
//    @DubboReference(check = false,mock = "force:return gpwy")
    ...
    public void test2() throws InterruptedException {
        RpcContext.getServerAttachment().setAttachment("return",false);
        userService.doKill("WUYA");
        Thread.currentThread().join();
    }
    ...
    public void async() throws InterruptedException {
        ...
        new Thread(()->{
            ...
            System.out.println(Thread.currentThread().getName() + "main==" + aa);
            System.out.println(Thread.currentThread().getName() + "并行调用其他接口====");
            try {
                Thread.sleep(2000);
            ...
            resultFuture.whenComplete((retValue,exception)->{
                if(exception == null) {
                    System.out.println(Thread.currentThread().getName() + "正常返回==" + retValue);
                } else {
